...
import { quarry } from "@/content/quarry";
...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/quarry">): Promise<Metadata> {
  ...
  return {
    ...
    alternates: buildAlternates("/quarry", locale),
  };
}

export default async function QuarryPage({ params }: PageProps<"/[locale]/quarry">) {
  const locale = await resolveLocale(params);
  setRequestLocale(locale);

  const t = await getTranslations("quarry");
  ...
  return (
    <>
      {/* Фонове фото з оригіналу (.main-section.carer-image) плюс градієнт —
          без нього білий текст на світлих ділянках знімка не читався б. */}
      <section className="relative isolate overflow-hidden bg-ink bg-[url('/images/quarry/hero.jpg')] bg-cover bg-center py-15 text-white md:py-20">
        <div
          aria-hidden="true"
      ...
      <section className="py-15 md:py-20">
        <Reveal className="container-page">
          ...
          <div className="grid grid-cols-1 gap-x-16 gap-y-5 lg:grid-cols-2">
            {quarry.intro.map((paragraph, index) => (
      ...
      <section className="pb-15 md:pb-20">
        <div className="container-page">
          <h2 className="text-h2 text-ink">{quarry.potential.title[activeLocale]}</h2>
          ...
          >
            {quarry.potential.stats.map((stat) => (
              <li
                key={stat.title.uk}
      ...
      {/* Той самий блок «зображення + текст», що в лістингу каталогу. */}
      <MediaTextSection
        image={quarry.location.image}
        imageAlt={quarry.potential.title[activeLocale]}
        imageSide="left"
        tone="dark"
      >
        <h2 className="text-h2">{quarry.location.title[activeLocale]}</h2>
        <p className="mt-5 leading-relaxed text-gainsboro">
          {quarry.location.body[activeLocale]}
        </p>
